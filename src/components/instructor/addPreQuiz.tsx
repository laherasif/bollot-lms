...
export default ({ onStepChange, onPrevStep, step }: any) => {
  ...
  const removeInputFields = (index: number, i: number) => {
    ...
    if (errors) {
      const error: any = [...errors.questions];
  ...
  const UpdateQuiz = async () => {
    ...
    try {
      setSaveQuiz(true)
      let res = await AxInstance.post('api//instructor/courses/quiz/store/1', value)
      ...
      <div className="complete-web-1 mb-3 w-100 " style={{ marginBottom: '60px' }}>
        ...
        {PreQuiz && PreQuiz.length ? PreQuiz?.map((q: any, index: number) => (
          <div className="p-3 quiz" id="quiz-card" key={index}
            style={errors && errors.questions && errors.questions[0]?.options ? { width: '100%', border: '1pt solid red' } : { width: '100%' }}
          >
            ...
            <input
              ...
              name="question"
              className="w-100 "
              style={errors && errors.questions && errors.questions[index]?.question && { border: '1pt solid red' }}
              value={q.question}
              onChange={(e) => handleChange(index, e)}
              placeholder="Write here..."
            />
            {errors && errors.questions && errors.questions[index]?.question ? <div className="invalid mt-1">{errors.questions[index]?.question}</div> : null}
            ...
            {q.options.length ? q.options.map((op, i) => (
                ...
                <div
                  ...
                  <div className="input_fields">
                    <input
                      type="text"
                      name="option"
                      style={errors && errors.questions && errors.questions[index]?.options[i]?.option && { border: '1pt solid red' }}
                      value={op.option}
                      onChange={(e) => handleChangeOptions(index, i, e)}
                      placeholder="Write here...."
                    />

                    {errors && errors.questions && errors.questions[index]?.options ? <div className="invalid mt-1 w-100">{errors && errors.questions[index]?.options[i]?.option}</div> : null}
                  </div>
              ...
              :
              <div className="ml-3">
                {errors && errors.questions ? <div className="invalid mt-1">{errors.questions[index]?.options[0]}</div> : null}
              </div>
