...
const Home: NextPage = () => {
  ...
  useEffect(() => {
    let fetchQuiz = async () => {
      try {
        setLoading(true)
        let res = await AxInstance.get(`api//instructor/courses/quiz/${courseId}/1`)
        if (res.data.success === true && res.data.response.course_with_quiz) {
                    ...
                    {allQuiz.length ? allQuiz?.map((q, index) => (
                      <div className="p-3 quiz" key={index} id="quiz-box">
                        ...
                        <div>
                          ...
                          {/* {errors && errors[0]?.options && errors[0]?.options? <div className="invalid mt-1">{errors[0]?.options}</div> : null} */}
                        </div>
